...
socket.on("init", (data) => {
  const { messages: messagesNormalized, compression } = data;
  ...
  if (messages) {
    ...
    messages.forEach((message) => {
      messagesRows += buildMessage(message._doc);
    });
...
socket.on("new-message", (newMessage) => {
  const { compression, message } = newMessage;

  const messageParagraph = buildMessage(message);
...
const buildMessage = (message) => {
  return `
      <p class="mb-1">
        <small class="text-primary">${message.author.alias}</small>
        <small class="text-danger">${message.createdAt}</small>
        <span class="text-success">${message.text}</span>
        <img src="${message.author.avatar}" width="30" height="30">
      </p>
    `;
};
